...
function navegar(abaId) {
    ...
    document.querySelectorAll('.tab-content').forEach(secao => {
        ...
        secao.style.display = 'none'; // Garante que suma mesmo
    });

    // 2. Mostra APENAS a seção que você clicou
    ...
    // 3. Atualiza o visual dos botões na barra inferior
    document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
    // Opcional: Adicionar classe active no botão clicado se desejar destaque
    
    // 4. Volta para o topo para não começar a página no meio
